                ...
                if is_cart_item_exists:
                    ...
                    for pr in product_variation:
                        if pr in ex_var_list:
                            ...
                            item_id_pv = cart_item_id_list_cart_select[index_pv]
                            print('commont item found - item_id_pv: ', item_id_pv)
                            item_pv = CartItem.objects.get(id=item_id_pv)
                            qty = item_pv.quantity
                            print('commont item found - item_pv.quantity: ', qty)
                            item_pv.delete()

                            # correct quantity of common cart_item
                            item.quantity += qty
                            print('commont item found - quantity: ', item.quantity)
                            item.user = user
                            print('commont item found - user: ', item.user)
                            item.save()
                        else:
                            cart_item = CartItem.objects.filter(cart=cart)
                            for item in cart_item:
